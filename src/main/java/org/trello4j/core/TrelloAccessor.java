...
class TrelloAccessor {
    ...
    <T> T doGet(String uri, Map<String, ?> data, ParameterizedTypeReference<T> typeReference) {
        try {
            return restTemplate.exchange(buildUri(uri, data), HttpMethod.GET, HttpEntity.EMPTY, typeReference)
                    .getBody();
        } catch (Exception e) {
            return null;
        }
    ...
    <T> T doPost(String uri, Map<String, ?> data, Class<T> responseType) {
        try {
            return restTemplate.exchange(buildUri(uri, data), HttpMethod.POST, HttpEntity.EMPTY, responseType)
                    .getBody();
        } catch (Exception e) {
            e.printStackTrace();
            return null;
    ...
    <T> T doPut(String uri, Map<String, ?> data, ParameterizedTypeReference<T> typeReference) {
        try {
            return restTemplate.exchange(buildUri(uri, data), HttpMethod.PUT, HttpEntity.EMPTY, typeReference)
                    .getBody();
        } catch (Exception e) {
            e.printStackTrace();
            return null;
